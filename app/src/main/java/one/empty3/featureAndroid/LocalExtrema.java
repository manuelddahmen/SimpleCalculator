...
public class LocalExtrema extends FilterMatPixM {
    ...
    @Override
    public one.empty3.matrix.M3 filter(M3 original) {
        one.empty3.matrix.M3 copy = new one.empty3.matrix.M3(original.getColumns(), original.getLines(),
                1, 1);
    ...
    private double lambda1dot2div1sum2(one.empty3.matrix.M3 original, int compNo, int i, int j) {
        one.empty3.matrix.PixM pixM = new PixM(3, 3);
    ...
    @Override
    public one.empty3.matrix.M3 norm(one.empty3.matrix.M3 m3, one.empty3.matrix.M3 copy) {
        return m3.copy();
    }

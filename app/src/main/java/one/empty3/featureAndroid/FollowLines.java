...
public class FollowLines {
    private one.empty3.matrix.M3 traces;
    private double threshold = 0.1;
    private ArrayList<Line> lines = new ArrayList<>();
    private int maxNeighbours = 2;

    public FollowLines(one.empty3.matrix.M3 traces) {
        this.traces = traces;
    }

    public one.empty3.matrix.M3 getTraces() {
        return traces;
    }

    public void setTraces(one.empty3.matrix.M3 traces) {
        this.traces = traces;
    }
    ...
    public ArrayList<Line> processPoints(int ii, int ij) {
        ...
        M3 points = new one.empty3.matrix.M3(traces.getColumns(), traces.getLines(), 1, 1);

...
import one.empty3.libraryAndroid.Polygon1;
import one.empty3.libraryAndroid.Serialisable;
...
public class GoogleFaceDetection
            ...
            public Object createFromParcel(Parcel parcel) {
                ...
                for (int face = 0; face < numFaces; face++) {
                    ...
                    for (int i = 0; i < numPoly; i++) {
                        Polygon1 polygon1 = Polygon1.CREATOR.createFromParcel(parcel);
                        PixM contours = one.empty3.matrix.PixM.CREATOR.createFromParcel(parcel);
                        int colorFill = parcel.readInt();
                        int colorContours = parcel.readInt();
                        int colorTransparent = parcel.readInt();

                        faceData.getFaceSurfaces().add(new FaceData.Surface(id, polygon1, contours, colorFill, colorContours,
                                colorTransparent));
    ...
    public void writeToParcel(@NonNull Parcel parcel, int i) {
        ...
        for (int face = 0; face < getDataFaces().size(); face++) {
            ...
            for (int j = 0; j < faceData.getFaceSurfaces().size(); j++) {
                ...
                parcel.writeParcelable(surface.polygon1, 1);
    ...
    public static class FaceData implements Serializable {
        ...
        public static class Surface implements Serializable, Serialisable{
            ...
            private Polygon1 polygon1;
            private PixM contours;
            private PixM filledContours;
            @Nullable
            public PixM actualDrawing;

            public Surface(int surfaceId, Polygon1 polygon1, PixM contours, int colorFill, int colorContours, int colorTransparent) {
            ...
            public void setPolygon1(Polygon1 Polygon1) {
                this.polygon1 = polygon1;
            }
            ...
            public void computeFilledSurface(PointF position, double scale) {
                polygon1.texture(new ColorTexture(colorFill));
                filledContours = polygon1.fillPolygon2D(this, null, contours.getBitmap(),
                        colorTransparent, 0, position, scale);
            }

            public boolean isContaning(Point pInPicture) {
                StructureMatrix<Point3D> boundRect2d = polygon1.getBoundRect2d();

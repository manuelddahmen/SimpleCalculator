...
import one.empty3.featureAndroid.FilterPixM;
...
public class M3 {
    ...
    public M3(one.empty3.matrix.PixM pixM[][]) {
    ...
    public one.empty3.matrix.PixM[][] getImagesMatrix() {
        return normalize(0.0, 1.0);
    }

    public one.empty3.matrix.PixM[][] normalize(final double min, final double max) {
        one.empty3.matrix.PixM[][] res = new one.empty3.matrix.PixM[columnsIn][linesIn];
        ...
        for (int ii = 0; ii < columnsIn; ii++) {
            for (int ij = 0; ij < linesIn; ij++) {
                ...
                one.empty3.matrix.PixM image = new PixM(columns, lines);
                res[ii][ij] = image;
            }
        }
    ...
    public one.empty3.matrix.PixM getMatrix(int ii, int ij) {
        one.empty3.matrix.PixM matrix = new PixM(columns, lines);
        for (int i = 0; i < columns; i++) {
            for (int j = 0; j < lines; j++) {
                for (int c = 0; c < getCompCount(); c++) {
                    setCompNo(c);

...
public class LocalExtrema extends FilterMatPixM {
    ...
    @Override
    public matrix.M3 filter(M3 original) {
        matrix.M3 copy = new matrix.M3(original.getColumns(), original.getLines(),
                1, 1);
    ...
    private double lambda1dot2div1sum2(matrix.M3 original, int compNo, int i, int j) {
        matrix.PixM pixM = new PixM(3, 3);
        pixM.setRegionCopy(original, 0, 0, i - 1, j - 1, i + 1, j + 1, pixM, 0, 0);
        return pixM.determinant() / pixM.diagonalSum();
    }

    @Override
    public void element(matrix.M3 source, matrix.M3 copy, int i, int j, int ii, int ij) {

    }

    @Override
    public matrix.M3 norm(matrix.M3 m3, matrix.M3 copy) {
        return m3.copy();
    }

...
public class Classification extends ProcessFile {
    ...
    public boolean process(File in, final File out) {
        ...
        selectPointColorMassAglo = matrix.PixM.getPixM(read, maxRes);
        ...
        for (int i = 0; i < imageOut.getWidth(); i += 1)
            for (int j = 0; j < imageOut.getHeight(); j += 1) {
                selectPointColorMassAglo1.setTmpColor(one.empty3.libs.Color.valueOf((float) Math.random(), (float) Math.random(), (float) Math.random()));

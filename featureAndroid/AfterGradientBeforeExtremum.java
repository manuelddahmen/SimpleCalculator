...
public class AfterGradientBeforeExtremum extends FilterMatPixM {
    ...
    @Override
    public matrix.M3 filter(matrix.M3 maximumAndGradient00m01gx02gy) {
        matrix.M3 original = maximumAndGradient00m01gx02gy;
...
//        M3 filtered = new GradientFilter(original.getColumns(), original.getLines()).filter(original);
        matrix.M3 filtered = original;
        int ii;
        int ij = 0;
        double itere = angles;

        matrix.M3 orientations = new matrix.M3(original.getColumns(), original.getLines(),
                1, 1);
    ...
    @Override
    public matrix.M3 norm(matrix.M3 m3, M3 copy) {
        return null;
    }

...
public class Lines3 extends ProcessFile {
    ...
    public boolean process(File in, File out) {
        ...
        matrix.PixM o = new matrix.PixM(pixM.getColumns(), pixM.getLines());
...
//
        ...
        lists2.forEach(p3s -> {
            android.graphics.Color r = new Color(Lumiere.getIntFromFloats((float) r(), (float) r(), (float) r()));
            double xA;
            double yA;
            p3s.forEach(point3D -> {
                if (isInBound(point3D)) {
        ...
        PixM img3 = new matrix.PixM(pixM.getColumns(), pixM.getLines());
        list3.forEach(p3s -> {
            android.graphics.Color r = new Color(Lumiere.getIntFromFloats((float) r(), (float) r(), (float) r()));
            double xA;
            double yA;
        ...
        for (LineSegment line : lines) {
            ...
            if (line.getLength() >= 2) {
                ...
                if (isInBound(pDraw1) && isInBound(pDraw2)) {
                    bLines.setRgb(x1, y1, android.graphics.Color.WHITE);
                    bLines.setRgb(x2, y2, android.graphics.Color.WHITE);
                }
            }
        }
...
//            });
       ...
       one.empty3.ImageIO.write(img3.normalize(0.0, 1.0).getImage(), "jpg",

...
public class Transform1 extends ProcessFile {
    ...
    public boolean process(File in, File out) {
        ...
        matrix.M3 imgForGrad = new matrix.M3(pixMOriginal, 2, 2);
        matrix.M3 filter = gradientMask.filter(imgForGrad);
...
//logger.info("angles tangentes");
   ...
   WriteFile.writeNext("gradients dot"+file.getName(), smoothedGrad.normalize(0.,1.).getImage());
     ...
     */
        try {
           one.empty3.ImageIO.write(plext3.getImage(), "jpg", out, shouldOverwrite);
        } catch (Exception ex) {
            return false;
        }
